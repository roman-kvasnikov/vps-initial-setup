...
# ═══════════════════════════════════════════════════════════════
# ЭТАП 9: Настройка Firewall (nftables)
# ═══════════════════════════════════════════════════════════════
header "ЭТАП 9: Настройка Firewall (nftables)"
...
# inet = IPv4 + IPv6 в одной таблице, ip = только IPv4
if [[ "$DISABLE_IPV6" == "y" ]]; then
    NFT_FAMILY="ip"
else
    NFT_FAMILY="inet"
...
info "Генерация /etc/nftables.conf (семейство: $NFT_FAMILY)..."
...
echo "        # ICMPv6 (Neighbor Discovery, Path MTU и т.д.)"
...
if systemctl restart ssh 2>/dev/null || systemctl restart sshd 2>/dev/null; then
    ...
    if nft list chain $NFT_FAMILY filter input 2>/dev/null | grep -q "dport $SSH_PORT"; then
        success "nftables: порт $SSH_PORT открыт ✔"
    else
        error "nftables: порт $SSH_PORT НЕ найден в правилах!"
        warn "Экстренное открытие порта..."
        nft add rule $NFT_FAMILY filter input tcp dport "$SSH_PORT" accept
...
echo -e "  ├─ ✔ IPv6: ${GREEN}$([ "$DISABLE_IPV6" == "y" ] && echo "отключён (family: ip)" || echo "включён (family: inet)")${NC}"
echo -e "  ├─ ✔ IP forwarding: ${GREEN}$([ "$ENABLE_IP_FORWARD" == "y" ] && echo "включён" || echo "отключён")${NC}"
echo -e "  ├─ ✔ nftables Firewall + SSH rate limiting (4 conn/min)"
