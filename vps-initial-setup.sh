...
PACKAGES=(
    ...
    logwatch           # [УЛУЧШЕНИЕ] Сводка логов на email
    needrestart        # [УЛУЧШЕНИЕ] Проверка нужен ли рестарт сервисов
    apt-listchanges    # [УЛУЧШЕНИЕ] Показ изменений при обновлении
)
...
# В Ubuntu 22.04+ Include стоит в начале sshd_config, поэтому наш файл имеет приоритет
...
# [УЛУЧШЕНИЕ] Отключение X11 forwarding (не нужен на сервере)
X11Forwarding no

# [УЛУЧШЕНИЕ] Ограничение максимума попыток аутентификации
MaxAuthTries 3

# [УЛУЧШЕНИЕ] Ограничение одновременных неаутентифицированных соединений
MaxStartups 3:50:10
...
# ═══════════════════════════════════════════════════════════════
header "ЭТАП 9: Настройка Firewall (UFW)"

# Отключаем IPv6 в UFW если нужно
if [[ "$DISABLE_IPV6" == "y" ]]; then
    sed -i 's/^IPV6=.*/IPV6=no/' /etc/default/ufw
    success "IPv6 отключён в UFW"
fi
...
# Разрешаем SSH на новом порту с rate limiting
ufw limit "$SSH_PORT/tcp" comment "SSH rate limit"

# Включаем UFW
ufw --force enable
success "Firewall включён. Открыт порт: $SSH_PORT/tcp (SSH)"
...
findtime  = 600
bantime   = 3600
# [УЛУЧШЕНИЕ] Увеличиваем время бана при повторных нарушениях
bantime.increment = true
bantime.factor    = 2
bantime.maxtime   = 86400
EOF

# [УЛУЧШЕНИЕ] Защита от сканирования портов (recidive jail)
...
# Настраиваем без интерактивного режима
...
if [[ "$INSTALL_ENDLESSH" == "y" ]]; then
    ...
    if apt-cache show endlessh &>/dev/null; then
        ...
        # Пакетный unit-файл содержит PrivateUsers=true и InaccessiblePaths,
        # которые мешают слушать порт 22 — создаём override
...
EOF
        ...
        # Создаём systemd unit (без sandbox-ограничений)
...
echo -e "  ├─ ✔ IPv6: ${GREEN}$([ "$DISABLE_IPV6" == "y" ] && echo "отключён" || echo "включён")${NC}"
echo -e "  ├─ ✔ IP forwarding: ${GREEN}$([ "$ENABLE_IP_FORWARD" == "y" ] && echo "включён" || echo "отключён")${NC}"
echo -e "  ├─ ✔ UFW Firewall с rate limiting (порт $SSH_PORT)"
